...
// ---------- template lookup (robust) ----------
const templateCostCache = new Map(); // key: String(templateId) -> { cost, currency, name }
const allTemplatesCache = { list: null, ts: 0 };

function keyId(id) { if (id == null) return null; const n = Number(id); return Number.isFinite(n) ? String(n) : String(id); }
function logOnce(k, ...args) { if (!DEBUG) return; if (logOnce[k]) return; logOnce[k]=1; console.log(...args); }

/** Primary by ID (tries both API shapes) */
async function fetchTemplateById(templateIdIn) {
  const key = keyId(templateIdIn);
  if (!key) return null;
  ...
  // 2) Alternate: getPrepaidPackageTemplate { prepaidPackageTemplateId }
  if (!tpl) {
    try {
  ...
  const cost = Number((tpl && (tpl.cost ?? tpl.price ?? tpl.amount ?? tpl.subscriberCost)) ?? NaN);
  const val = {
...
/** Fallback by name: fetch all templates once and find by name */
async function fetchAllTemplatesOnce() {
  const now = Date.now();
  if (allTemplatesCache.list && now - allTemplatesCache.ts < 10 * 60 * 1000) return allTemplatesCache.list;
  const r = await callOCS({ listPrepaidPackageTemplate: {} }); // many APIs return all on empty filter
  const list = r?.listPrepaidPackageTemplateRsp?.prepaidPackageTemplate ??
               r?.prepaidPackageTemplate ?? [];
  allTemplatesCache.list = Array.isArray(list) ? list : [];
  allTemplatesCache.ts = now;
  return allTemplatesCache.list;
}

async function fetchTemplateByNameMaybe(name) {
  if (!name) return null;
  try {
    const all = await fetchAllTemplatesOnce();
    const found = all.find(t =>
      String(t.prepaidpackagetemplatename ?? t.name ?? t.templateName ?? "")
        .toLowerCase() === String(name).toLowerCase()
    );
    if (!found) return null;
    const cost = Number((found.cost ?? found.price ?? found.amount ?? found.subscriberCost) ?? NaN);
    return {
      cost: Number.isFinite(cost) ? cost : null,
      currency: found.currency ?? found.curr ?? null,
      name: found.prepaidpackagetemplatename ?? found.name ?? found.templateName ?? null
    };
  } catch { return null; }
}
...
export async function fetchAllData(accountIdParam) {
  ...
  const rows = subscribers.map((s) => {
    ...
    // 🔎 also try to pull template info directly from listSubscriber object (fallback)
    const subTplName =
      s?.prepaidpackagetemplatename ?? s?.prepaidPackageTemplateName ?? null;
    const subTplId =
      s?.prepaidpackagetemplateid ??
      s?.prepaidPackageTemplateId ??
      s?.templateId ?? null;
    ...
    return {
      ...
      // package (may be filled via listSubscriberPrepaidPackages or already here)
  ...
  await pMap(rows, async (r) => {
    ...
    // 1) Try to enrich from latest package (gives us templateId if missing)
    try {
      const pkg = await fetchPackagesFor(r._sid);
      if (pkg) {
        // Only fill if missing so we don't overwrite already-present fields
    ...
    // 2) Template cost by ID (primary)
    try {
      if (r.prepaidpackagetemplateid != null) {
        const tpl = await fetchTemplateById(r.prepaidpackagetemplateid);
      ...
      logOnce(`tpl_id_fail_${r._sid}`, "[ROW] template-by-ID failed", r._sid, e?.message);
    }

    // 3) Fallback: if still no cost but we have a name, try by name
    try {
      if (r.subscriberOneTimeCost == null && r.prepaidpackagetemplatename) {
        const tplByName = await fetchTemplateByNameMaybe(r.prepaidpackagetemplatename);
        if (tplByName?.cost != null) r.subscriberOneTimeCost = tplByName.cost;
      }
    } catch (e) {
      logOnce(`tpl_name_fail_${r._sid}`, "[ROW] template-by-name failed", r._sid, e?.message);
    }

    // 4) Aggregated usage & reseller cost (Jun 1 → today)
    try {
